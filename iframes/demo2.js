...
window.onload = function() {
    ...
    function labelPoint(p, name) {
        ...
        context.fillText("x: " + Math.round(p.x), p.x + 10, p.y + 35);
        context.fillText("y: " + Math.round(p.y), p.x + 10, p.y + 55);
    }

    function labelPointLeft(p, name) {
        context.fillStyle = "gray";
        context.fillText(name, p.x - 40, p.y - 55);
        context.fillText("x: " + Math.round(p.x), p.x - 40, p.y - 30);
